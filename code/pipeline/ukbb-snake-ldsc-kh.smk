...
pdvcf = "/dartfs/rc/lab/S/Szhao/katieh/organoid-rotation/code/" # contains vcf preprocessing script
...
pdout = "/dartfs/rc/lab/S/Szhao/katieh/organoid-rotation/outputs/UKBB/" # desired output folder

# Inputs (summary stats, a directory of annotations)
raw_sumstats = "/dartfs/rc/lab/S/Szhao/katieh/organoid-rotation/raw-sumstats/UKBB/"
cleaned_sumstats = "/dartfs/rc/lab/S/Szhao/katieh/organoid-rotation/cleaned-sumstats/UKBB/" 
...
COLUMNS = "CHROM,POS,ES,SE,REF,ALT,ID,LP"

rule all:
  input:
      expand(results + '{t}_enrichment.results', t=trait),
      expand(results + '{t}_signif_loci.txt', t=trait),
      expand(cleaned_sumstats + '{t}_sumstats.txt.gz', t=trait),
      expand(raw_sumstats + 'ukb-d-{t}_irnt_preprocessed.vcf.gz', t=trait)
      # expand(results + "{t}/{t}.{c}.snpvar_ridge.gz",t=trait, c=chrom),
      # expand(results + "{t}/{t}.{c}.snpvar_ridge_constrained.gz", t=trait, c=chrom),
      # expand(finemapping + 'processed/{t}_finemapped_susie_L1.txt.gz',t=trait)


rule vcf_preprocess:
  input:
      raw_sumstats + 'ukb-d-{trait}_irnt.vcf.gz'
  output:
      raw_sumstats + 'ukb-d-{trait}_irnt_preprocessed.vcf.gz'
  shell:
    """
    bash {pdvcf}vcf_openGWAS_preprocess.sh {input} {output}
    """
...
rule clean_sumstats:
    ...
    shell:
        """
        Rscript {pdnew}clean_sumstats_ukbb.R {input} {params.columns} {output}
        """
